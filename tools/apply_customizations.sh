...
check_variant() {
    local file="$1"
    if grep -qF "$VARIANT" "$file"; then return 0; fi          # already ours
    if grep -qF "$MARKER" "$file"; then
        die "$(basename "$file") was customized for a different variant. Restore the generated files (git checkout -- '$LAYOUT_DIR') and re-run."
    fi
    return 1
}
...
if check_variant "$RULES"; then
    skipped "rules.mk"
else
    grep -q '^ORYX_ENABLE[[:space:]]*=[[:space:]]*yes' "$RULES" \
        || die "anchor missing in rules.mk: expected 'ORYX_ENABLE = yes'. Oryx output changed."
    if [ "$OS_DETECTION" -eq 1 ]; then OS_LINE="OS_DETECTION_ENABLE = yes"
    else OS_LINE="# OS_DETECTION_ENABLE = yes   # needed by BASE_LAYERS_OS_DETECTION (see config.h)"; fi
